    ...
    [(Decl_func
        ("main",
         { args = []; returns = None;
           ...
           [(Stmt_call
               ((Expr_ident "println"),
                [(Expr_const (Const_int 1)); (Expr_const (Const_int 1));
                  (Expr_const (Const_string "k")); (Expr_const (Const_int 3))]))
             ]
           }));
      (Decl_func
         ("swap",
          { args = [];
            returns = (Some (Only_types (Type_string, [Type_string])));
            body =
            [(Stmt_return
                [(Expr_const (Const_string "a")); (Expr_const (Const_string "b"))
                  ])
              ]
            }));
      (Decl_func
         ("println",
          { args = [("a", Type_string); ("b", Type_string); ("c", Type_string)];
            returns =
            (Some (Only_types (Type_string, [Type_string; Type_string])));
            body =
            [(Stmt_return [(Expr_ident "a"); (Expr_ident "b"); (Expr_ident "c")])
              ]
            }))
      ]  |}]
;;
